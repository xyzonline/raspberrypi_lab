...
import wiringpi
owpin=8     #第8脚为1-wire脚 ,wiringpi的引脚图
runned = 0

# http://www.cnblogs.com/yafengabc/p/5100741.html
...
def getval(owpin):
    ...
    tb=[]  #存放数据位
    if runned == 0:
        gpio.wiringPiSetup() #初始化wiringpi库 ,只初始化一次 http://stackoverflow.com/questions/29198312/error-on-wiringpi
...
def GetResult(output="temperature",owpin=owpin):
    for i in range(10):
...
while 1:
  try:
    ...
    print u"湿度:",SH,SL,u"温度:",TH,TL #(SH,TH) #温度 湿度
    #湿度: 63 0 温度: 23 0
  except:
    pass
    #SH,SL,TH,TL=GetResult(owpin)
